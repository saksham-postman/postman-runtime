        ...
        event (payload, next) {
            ...
            async.mapSeries(events, function (event, next) {
                ...
                this.host.on(EXECUTION_VAULT_BASE + executionId, async function (id, cmd, ...args) {
                    console.log('coming inside EXECUTION_VAULT_BASE');

                    if (typeof isVaultAccessInScriptsAllowed === 'function') {
                        allowVaultAccess = await isVaultAccessInScriptsAllowed(item.id);
                    }
                    else {
                        allowVaultAccess = isVaultAccessInScriptsAllowed;
                    }
                    console.log('allowVaultAccess inside EXECUTION_VAULT_BASE', allowVaultAccess);
